...
bool HasChanged(float val, float oldVal) {
	return abs(oldVal - val) > 0.01; // this delta value is difficult
}

void SetReverbCutoff(float val) {
...
void UpdateKnobs()
{
	...
	if(outerMode == MODE_PLAY) {
		if (HasChanged(k1, k1old)) {
			switch (innerMode) {
				case MODE_SNARE: snareEnv.SetTime(ADENV_SEG_DECAY, k1*10.0); break;
				case MODE_DRIVE: drive.SetDrive(k1); break;
				case MODE_REVERB: rev.SetFeedback(k1); break;
				case MODE_TEMPO: SetTempo(k1);
				default: kickVolEnv.SetTime(ADENV_SEG_DECAY, k1*10.0); break;
			}
		...
		if (HasChanged(k2, k2old)) {
			switch (innerMode) {
				case MODE_SNARE: snareEnv.SetTime(ADENV_SEG_ATTACK, k2*10.0); break;
				case MODE_DRIVE: SetDrywet(k2); break; // kind of a silly place to put it but alas
				case MODE_REVERB: SetReverbCutoff(k2); break;
				case MODE_TEMPO: break;
				default: kickVolEnv.SetTime(ADENV_SEG_ATTACK, k2*10.0); break;
			}
...
void UpdateButtons() {
	if(outerMode == MODE_EDIT) {
		...
		} else if (hardware.button2.RisingEdge()) {
